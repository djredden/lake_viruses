...
data_cleaned <- left_join(raw_data, std_curve_data, by = "target") %>% 
  ...
  mutate(gu_sample = case_when(
    ...
    adsorbent == "gac" & extraction %in% extraction_types ~ gu_lysate * (eluent_vol_ml * 1000 + lysis_vol_ul)  / ads_mass_mg,
  ...
  mutate(sample_id = factor(sample_id, levels = c("banook1", "banook2")),
         sample_id = if_else(sample_id == "banook1", "Banook 1", "Banook 2")) %>% 
